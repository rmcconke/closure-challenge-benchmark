"""Compare Periodic Hill Re_5600 00Baseline (SST) against LES data."""
...
def modeled_reynolds_shear(profile: dict[str, np.ndarray], u_ref: float) -> np.ndarray:
    ...
    if not all(key in profile for key in ("k", "omega", "Ux", "y")):
        missing = ", ".join(
            key for key in ("nut", "dUx/dy", "dUy/dx", "k", "omega", "Ux", "y")
    ...
    nut = np.divide(
        profile["k"],
        profile["omega"],
        out=np.full_like(profile["k"], np.nan),
        where=profile["omega"] != 0.0,
    )
    return -nut * np.gradient(profile["Ux"], profile["y"]) / u_ref**2
...
def main() -> None:
    ...
    write_csv(
        f"{figures_dir}/wall_Cp_Cf_PeriodicHill_Re5600.csv",
    ...
    for wall_label, file_label, sst, les in (
        ...
        fig.tight_layout()
        fig.savefig(f"{figures_dir}/compareCp_{file_label}_PeriodicHill_Re5600.pdf")
        plt.close(fig)
        ...
        ax.legend(loc="best", fontsize=11)
        fig.tight_layout()
        fig.savefig(f"{figures_dir}/compareCf_{file_label}_PeriodicHill_Re5600.pdf")
        plt.close(fig)
    ...
    ax.legend(loc="best")
    fig.tight_layout()
    fig.savefig(f"{figures_dir}/compareUxProfiles_PeriodicHill_Re5600.pdf")
    plt.close(fig)
    ...
    ax.legend(loc="best")
    fig.tight_layout()
    fig.savefig(f"{figures_dir}/compareuvProfiles_PeriodicHill_Re5600.pdf")
    plt.close(fig)

    write_csv(
        f"{figures_dir}/velocity_shear_profiles_PeriodicHill_Re5600.csv",
        [
            "station_x_over_h",
            "y_over_h",
    ...
    for file_name in (
        "compareCp_bottomWall_PeriodicHill_Re5600.pdf",
        "compareCf_bottomWall_PeriodicHill_Re5600.pdf",
        "compareCp_topWall_PeriodicHill_Re5600.pdf",
        "compareCf_topWall_PeriodicHill_Re5600.pdf",
        "compareUxProfiles_PeriodicHill_Re5600.pdf",
        "compareuvProfiles_PeriodicHill_Re5600.pdf",
    ):
    ...
    print(f" - {figures_dir}/wall_Cp_Cf_PeriodicHill_Re5600.csv")
    print(f" - {figures_dir}/velocity_shear_profiles_PeriodicHill_Re5600.csv")
